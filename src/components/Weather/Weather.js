...
class Weather extends Component {
  ...
  componentDidMount() {
    ...
    navigator.geolocation.getCurrentPosition(position => {
        fetch("https://cors-anywhere.herokuapp.com/https://api.darksky.net/forecast/" + API_KEY + "/" + position.coords.latitude +  "," + position.coords.longitude+"?units=si")
        .then(response=>{
              ...
              .map((user,i) => {
                if(i===0){
                  if(defaultTemperature && defaultWindSpeed){
                      ...
                      return (
                        <div key={i*3} className = "card container">
                          <h3>Weather now</h3>
                  ...
                  }else if (!defaultTemperature && defaultWindSpeed){
                    ...
                    return (
                      ...
                      <div key={i*3} className = "card container">
                        <h3>Weather now</h3>
                        <h3>icon: {result[i].icon}</h3>
                        <h3>tempreature: {((result[i].temperature*1.8)+32).toFixed(2)} F</h3>
                        <h3>wind speed: {result[i].windSpeed.toFixed(2)} m/s</h3>
                      </div>
                    );
                    ...
                    return (
                      <div key={i*3} className = "card container">
                        ...
                        <h3>icon: {result[i].icon}</h3>
                        <h3>tempreature: {result[i].temperature.toFixed(2)} C</h3>
                        <h3>wind speed: {(result[i].windSpeed*3.6).toFixed(2)} km/h</h3>
                      </div>
                    );
                  ...
                  if(this.state.defaultTemperature && this.state.defaultWindSpeed){
